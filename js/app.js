const fetchEmployees = () => {
  fetch("assets/data/employee.json")
    ...
    .then((data) => {
      ...
      const idCount = +empDetails.length + 1;
  ...
  yesButton.addEventListener("click", () => {
    empDetails.forEach((empData) => {
      if (empData.id == empId) {
        ...
        localStorage.setItem("employData", JSON.stringify(empDetails));
        reloadTable();
        let rowsAfterDelete = document.querySelectorAll("#table-body tr");
        rowsAfterDelete.length === 0 && (errorDiv.style.display = "block");
      }
...
function generateFilteredTable() {
  ...
  applyButton.addEventListener("click", () => {
    ...
    if (checkedSkillArr.length) {
      ...
      !isNone && (errorDiv.style.display = "block");
    } else {
      reloadTable();
      errorDiv.style.display = "none";
    }
